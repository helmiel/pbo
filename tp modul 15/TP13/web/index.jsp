<%-- 
    Document   : index
    Created on : 8 Dec 2024, 11.15.00
    Author     : pc
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<%
    HttpSession mySession = request.getSession();
    if (session.getAttribute("JSP session") == null) {
        response.sendRedirect("product/session.jsp");
        return; // tambahkan return untuk menghentikan eksekusi kode selanjutnya
    }
%>

<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <title>Halaman Utama</title>
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
    </head>
    <body>
        <div class="vh-100">
            <div class="d-flex justify-content-center align-items-center h-100 flex-column text-center">
                <h1 class="fw-bold mb-4 text-dark">Selamat Datang!</h1>    
                <a href="product/view.jsp" class="btn btn-primary px-4 py-2" role="button">Daftar Produk</a>
            </div>
        </div>
        
        <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous"></script>
    </body>
</html>
